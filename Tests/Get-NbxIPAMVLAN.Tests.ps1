...
Describe "Get-NbxIPAMVLAN" {

    Context "Function structure and parameters" {

        It "Should be available as a function" {
            Get-Command -Name "Get-NbxIPAMVLAN" -ErrorAction SilentlyContinue | Should -Not -BeNullOrEmpty
        }

        It "Parameter names are PascalCase" {
            (Get-Command "Get-NbxIPAMVLAN").Parameters.Keys | ForEach-Object { $_ | Should -Match '^[A-Z].*' }
        }

        It "Mandatory parameters are present in parameter list" {
            $cmd = Get-Command "Get-NbxIPAMVLAN"
    ...
    Context "Function behavior" {
        ...
        It "Should call InvokeNbxRestMethod with Query parameter set" {
            Get-NbxIPAMVLAN -Query @{ name = 'Test' }
            Should -Invoke -ModuleName NetboxTools -CommandName InvokeNbxRestMethod -Times 1 -ParameterFilter {
                $Uri -like '*/ipam/vlans/*' -and $Method -eq 'GET' -and $Query.name -eq 'Test'
            }
        }
        ...
        It "Should call InvokeNbxRestMethod with Default parameter set" {
            Get-NbxIPAMVLAN
            Should -Invoke -ModuleName NetboxTools -CommandName InvokeNbxRestMethod -Times 1 -ParameterFilter {
                $Uri -like '*/ipam/vlans/*' -and $Method -eq 'GET'
            }
        }
