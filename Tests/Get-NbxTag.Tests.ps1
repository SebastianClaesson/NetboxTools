...
Describe "Get-NbxTag" {
    ...
    Context "Function structure and parameters" {
        ...
        It "Should be available as a function" {
            Get-Command -Name "Get-NbxTag" -ErrorAction SilentlyContinue | Should -Not -BeNullOrEmpty
        }

        It "Parameter names are PascalCase" {
            (Get-Command "Get-NbxTag").Parameters.Keys | ForEach-Object { $_ | Should -Match '^[A-Z].*' }
        }

        It "Mandatory parameters are present in parameter list" {
            $cmd = Get-Command "Get-NbxTag"
