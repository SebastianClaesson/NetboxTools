...
Describe "Get-NbxVirtualizationCluster" {

    Context "Function structure and parameters" {

        It "Should be available as a function" {
            Get-Command -Name "Get-NbxVirtualizationCluster" -ErrorAction SilentlyContinue | Should -Not -BeNullOrEmpty
        }

        It "Parameter names are PascalCase" {
            (Get-Command "Get-NbxVirtualizationCluster").Parameters.Keys | ForEach-Object { $_ | Should -Match '^[A-Z].*' }
        }

        It "Mandatory parameters are present in parameter list" {
            $cmd = Get-Command "Get-NbxVirtualizationCluster"
    ...
    Context "Function behavior" {
        ...
        It "Should call InvokeNbxRestMethod with Default parameter set" {
            ...
            Should -Invoke -ModuleName NetboxTools -CommandName InvokeNbxRestMethod -Times 1 -ParameterFilter {
                $Uri -like '*/virtualization/clusters/*' -and $Method -eq 'GET'
            }
        }
