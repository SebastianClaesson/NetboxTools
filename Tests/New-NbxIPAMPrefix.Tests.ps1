...
Describe "New-NbxIPAMPrefix" {

    Context "Function structure and parameters" {

        It "Should be available as a function" {
            Get-Command -Name "New-NbxIPAMPrefix" -ErrorAction SilentlyContinue | Should -Not -BeNullOrEmpty
        }

        It "Parameter names are PascalCase" {
            (Get-Command "New-NbxIPAMPrefix").Parameters.Keys | ForEach-Object { $_ | Should -Match '^[A-Z].*' }
        }

        It "Mandatory parameters are present in parameter list" {
            $cmd = Get-Command "New-NbxIPAMPrefix"
            $mandatoryParams = @()
            ...
            $mandatoryParams | Should -Contain 'Prefix'
        }
    }
    ...
    Context "Function behavior" {
        ...
        It "Should not throw with valid parameters" {
            { New-NbxIPAMPrefix -Prefix '10.0.0.0/24' } | Should -Not -Throw
        }
