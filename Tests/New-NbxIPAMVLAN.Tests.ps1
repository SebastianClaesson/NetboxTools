...
Describe "New-NbxIPAMVLAN" {

    Context "Function structure and parameters" {

        It "Should be available as a function" {
            Get-Command -Name "New-NbxIPAMVLAN" -ErrorAction SilentlyContinue | Should -Not -BeNullOrEmpty
        }

        It "Parameter names are PascalCase" {
            (Get-Command "New-NbxIPAMVLAN").Parameters.Keys | ForEach-Object { $_ | Should -Match '^[A-Z].*' }
        }

        It "Mandatory parameters are present in parameter list" {
            $cmd = Get-Command "New-NbxIPAMVLAN"
            $mandatoryParams = @()
            ...
            $mandatoryParams | Should -Contain 'VID'
