...
Describe "Remove-NbxDCIMDevice" {

    Context "Function structure and parameters" {

        It "Should be available as a function" {
            Get-Command -Name "Remove-NbxDCIMDevice" -ErrorAction SilentlyContinue | Should -Not -BeNullOrEmpty
        }

        It "Parameter names are PascalCase" {
            (Get-Command "Remove-NbxDCIMDevice").Parameters.Keys | ForEach-Object { $_ | Should -Match '^[A-Z].*' }
        }

        It "Mandatory parameters are present in parameter list" {
            $cmd = Get-Command "Remove-NbxDCIMDevice"
    ...
    Context "Function behavior" {
        ...
        It "Should call InvokeNbxRequest when Force is used" {
            Remove-NbxDCIMDevice -Id 1 -Force
        ...
        It "Should not throw with valid parameters" {
            { Remove-NbxDCIMDevice -Id 1 -Force } | Should -Not -Throw
        }
    }
