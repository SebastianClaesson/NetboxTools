...
Describe "Set-NbxDCIMDevice" {

    Context "Function structure and parameters" {

        It "Should be available as a function" {
            Get-Command -Name "Set-NbxDCIMDevice" -ErrorAction SilentlyContinue | Should -Not -BeNullOrEmpty
        }

        It "Parameter names are PascalCase" {
            (Get-Command "Set-NbxDCIMDevice").Parameters.Keys | ForEach-Object { $_ | Should -Match '^[A-Z].*' }
        }

        It "Mandatory parameters are present in parameter list" {
            $cmd = Get-Command "Set-NbxDCIMDevice"
            $mandatoryParams = @()
    ...
    Context "Function behavior" {
        ...
        BeforeAll {
            ...
            Mock -ModuleName NetboxTools -CommandName Get-NbxDCIMDevice -MockWith {
        ...
        It "Should call InvokeNbxRequest with PATCH method when Force is used" {
            Set-NbxDCIMDevice -Id 1 -Name 'Updated' -Force
            Should -Invoke -ModuleName NetboxTools -CommandName InvokeNbxRequest -Times 1
        }

        It "Should call Get-NbxDCIMDevice to verify the object exists" {
            Set-NbxDCIMDevice -Id 1 -Name 'Updated' -Force
            Should -Invoke -ModuleName NetboxTools -CommandName Get-NbxDCIMDevice -Times 1
        }

        It "Should not throw with valid parameters" {
            { Set-NbxDCIMDevice -Id 1 -Name 'Updated' -Force } | Should -Not -Throw
        }
    }
