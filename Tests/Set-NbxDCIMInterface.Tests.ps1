...
Describe "Set-NbxDCIMInterface" {

    Context "Function structure and parameters" {

        It "Should be available as a function" {
            Get-Command -Name "Set-NbxDCIMInterface" -ErrorAction SilentlyContinue | Should -Not -BeNullOrEmpty
        }

        It "Parameter names are PascalCase" {
            (Get-Command "Set-NbxDCIMInterface").Parameters.Keys | ForEach-Object { $_ | Should -Match '^[A-Z].*' }
        }

        It "Mandatory parameters are present in parameter list" {
            $cmd = Get-Command "Set-NbxDCIMInterface"
    ...
    Context "Function behavior" {
        ...
        It "Should call InvokeNbxRequest with PATCH method when Force is used" {
            Set-NbxDCIMInterface -Id 1 -Name 'Updated' -Force
        ...
        It "Should not throw with valid parameters" {
            { Set-NbxDCIMInterface -Id 1 -Name 'Updated' -Force } | Should -Not -Throw
        }
    }
